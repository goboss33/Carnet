  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        className={cn(
          "fixed left-1/2 top-1/2 z-50 flex max-h-[92dvh] w-[calc(100vw-1.5rem)] max-w-md -translate-x-1/2 -translate-y-1/2 flex-col rounded-xl border border-(--color-line) bg-white p-5 shadow-xl",
          className
        )}
        {...props}
      >
        <DialogPrimitive.Title className="shrink-0 text-[15px] font-semibold text-zinc-900">{title}</DialogPrimitive.Title>
        {desc ? <DialogPrimitive.Description className="mt-1 shrink-0 text-[13px] text-zinc-500">{desc}</DialogPrimitive.Description> : null}
