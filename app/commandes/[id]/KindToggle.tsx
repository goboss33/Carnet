...
export default function KindToggle({ orderId, exception }: { orderId: string; exception: boolean }) {
  ...
  return (
    <button
      ...
      title="Mode ligne : devis détaillé en lignes (B2B, grands événements, plusieurs gâteaux) — les relances grand public (anniversaire, avis) se désactivent pour cette commande."
    ...
    >
      ...
      <span className={exception ? "text-(--color-brand)" : "text-zinc-400"}>Mode ligne</span>
    </button>
  );
}
