...
function Lifecycle() {
  ...
  return (
    <div className="mb-5 overflow-x-auto rounded-xl border border-stone-200 bg-stone-50 px-4 py-3">
      <div className="flex min-w-max items-start">
        {LIFECYCLE.map((step, i) => (
          <div key={step} className="flex items-start">
            <div className="flex flex-col items-center gap-1.5">
              <span className="whitespace-nowrap text-[10px] font-bold uppercase tracking-wider text-stone-500">
                {step}
              </span>
            ...
            {i < LIFECYCLE.length - 1 && (
              <span className="mx-2 mt-4 h-px w-6 shrink-0 bg-stone-300 sm:w-9" aria-hidden />
            )}
          </div>
        ))}
      </div>
    ...
    <div className="rounded-xl border border-stone-200 px-4 py-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-[11px] font-medium text-amber-700">⏰ {a.trigger}</p>
        </div>
        {a.setting && (
      ...
      {a.delays?.length ? (
        <div className="mt-3 grid gap-3 border-t border-dashed border-stone-200 pt-3 sm:grid-cols-2">
          {a.delays.map((d) => (
            <label key={d.name} className="block">
              <span className="mb-1 block text-[11px] font-semibold text-stone-500">
                {d.label} {d.unit && <span className="text-stone-400">({d.unit})</span>}
              </span>
              <input
                name={d.name}
                type="number"
      ...
      {a.example && (
        <details className="mt-2">
          ...
          <pre className="mt-1.5 whitespace-pre-wrap rounded-lg bg-stone-100 px-3 py-2 font-sans text-xs text-stone-600">
            {a.example}
          </pre>
        </details>
      )}
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="mb-3 grid gap-4 sm:grid-cols-2">
          <label>
            <span className="mb-1 block text-[11px] font-semibold text-stone-500">Heure du digest matin (0-23)</span>
          ...
          <label>
            <span className="mb-1 block text-[11px] font-semibold text-stone-500">Heure des relances du soir (0-23)</span>
      ...
      <div>
        ...
        <div className="space-y-2.5">
          {reactions.map((a) => (
            <div key={a.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-stone-200 px-4 py-3">
              <span className="text-lg" aria-hidden>{a.emoji}</span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold">{a.name}</p>
                <p className="text-xs text-stone-500">{a.desc}</p>
      ...
      <div>
        ...
        <div className="overflow-hidden rounded-xl border border-stone-200">
          {commands.map((a, i) => (
            <div key={a.id} className={`flex items-center gap-3 px-4 py-2.5 ${i > 0 ? "border-t border-stone-100" : ""}`}>
              <span aria-hidden>{a.emoji}</span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold">
                  {a.name} <span className="ml-1 text-[11px] font-medium text-stone-400">{a.trigger}</span>
                </p>
                <p className="text-xs text-stone-500">{a.desc}</p>
              </div>
            </div>
