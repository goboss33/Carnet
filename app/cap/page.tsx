  ...
  return (
    <Shell>
      ...
      <div className="mb-8 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-xl border border-zinc-200 bg-white px-4 py-3.5">
            ...
            <p className="text-[11px] text-zinc-400">{k.sub}</p>
      ...
      <div className="mb-8 rounded-2xl border border-zinc-200 bg-white p-6">
        <div className="mb-4 flex items-baseline justify-between">
          <h2 className="text-sm font-bold uppercase tracking-wide text-zinc-600">CA livré — 12 mois</h2>
          <p className="text-xs text-zinc-400">— objectif {c.s.goalCaMensuel} CHF</p>
        </div>
          ...
          {c.caParMois.map((mo) => (
            <div key={mo.month} className="flex flex-1 flex-col items-center gap-1" title={`${mo.month} : CHF ${mo.ca} (net ${mo.net})`}>
              <div className="flex w-full flex-1 items-end">
                <div className={`w-full rounded-t ${mo.ca >= c.s.goalCaMensuel ? "bg-emerald-400" : "bg-zinc-300"}`} style={{ height: `${(mo.ca / maxCa) * 100}%` }} />
              </div>
              <span className="text-[10px] text-zinc-400">{moisLabel(mo.month)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {c.phases.map((p, i) => {
          ...
          return (
            <section key={p.name} className={`rounded-2xl border bg-white p-5 ${i === c.phaseCourante ? "border-amber-400 shadow-[0_0_0_3px_rgba(245,158,11,0.12)]" : "border-zinc-200"}`}>
              <div className="mb-3 flex items-baseline justify-between">
                <h2 className="text-sm font-bold">{p.name}</h2>
                <span className="text-xs font-bold text-zinc-400">{done}/{p.jalons.length}</span>
              </div>
              <ul className="space-y-2">
                {p.jalons.map((j) => (
                  <li key={j.key} className="flex items-start gap-2.5 text-sm">
                    {j.auto ? (
                      <span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[11px] font-bold ${j.done ? "bg-emerald-100 text-emerald-700" : "bg-zinc-100 text-zinc-400"}`}>
                        {j.done ? "✓" : "·"}
                      </span>
                    ) : (
                      <form action={toggleMilestone.bind(null, j.key, !j.done)}>
                        <button
                          ...
                          className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[11px] font-bold transition-colors ${j.done ? "bg-emerald-100 text-emerald-700" : "border border-zinc-300 text-zinc-300 hover:border-amber-500 hover:text-amber-600"}`}
                        >
                          {j.done ? "✓" : ""}
                        </button>
                      </form>
                    )}
                    <span className={j.done ? "text-zinc-700" : "text-zinc-500"}>
                      {j.label}
                      {j.detail && <span className="ml-1.5 text-[11px] text-zinc-400">({j.detail})</span>}
                    </span>
                  </li>
                ))}
              </ul>
