...
export default function ImportPage() {
  ...
  return (
    <>
      ...
      <form action={action} className="max-w-xl space-y-4 rounded-2xl border border-zinc-200 bg-white p-7">
        <input
          type="file"
          name="file"
          accept=".csv,text/csv"
          required
          className="block w-full text-sm text-zinc-600 file:mr-4 file:rounded-lg file:border-0 file:bg-zinc-900 file:px-4 file:py-2.5 file:text-sm file:font-semibold file:text-white hover:file:bg-zinc-700"
        />
      ...
      <details className="mt-8 max-w-xl rounded-2xl border border-red-200 bg-red-50/50 p-6">
        <summary className="cursor-pointer text-sm font-bold text-red-700">
          ⚠ Zone dangereuse — vider commandes & contacts
        </summary>
